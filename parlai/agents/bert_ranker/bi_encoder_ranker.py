...
class BiEncoderRankerAgent(TorchRankerAgent):
    ...
    def _set_text_vec(self, *args, **kwargs):
        ...
        if (
            ...
            obs.force_set(
                'label_vec', surround(obs['label_vec'], self.START_IDX, self.END_IDX)
            )
    ...
    def eval_step(self, batch):
        ...
        if np.isnan(scores.item()):
            return

        self.total_sim += sum(scores).item()
        self.count += scores.size(0)

        print(self.total_sim / self.count)

    def score_candidates(self, batch, cand_vecs=None, cand_encs=None):
        # Encode contexts first
        token_idx_ctxt, segment_idx_ctxt, mask_ctxt = to_bert_input(
            batch.text_vec, self.NULL_IDX
        )
        token_idx_cands, segment_idx_cands, mask_cands = to_bert_input(
            batch.label_vec, self.NULL_IDX
        )
        ...
        return self.cosine(embedding_ctxt, embedding_cands)
...
class BiEncoderModule(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.cand_encoder = self.context_encoder
        """
