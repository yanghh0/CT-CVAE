...
class History(object):
    ...
    def __init__(
        ...
        self.post_history_vecs = []
        self.post_turn_vecs = []
        self.post_role_vecs = []
        self.reverse_post_history_vecs = []
        self.reverse_post_turn_vecs = []
        self.reverse_post_role_vecs = []

        self.add_start_end_history_vecs = []
    ...
    def _update_vecs(self, text):
        ...
        _s = self.parse(text)
        self.history_vecs.append(_s)

        self.turn_vecs = [turn + 1 for turn in self.turn_vecs]
        self.turn_vecs += [1] * len(_s)
        self.role_vecs = []
        for turn_id in self.turn_vecs:
            if turn_id % 2 == 1:
                self.role_vecs.append(0)
            else:
                self.role_vecs.append(1)

    def _update_post_vecs(self, label):
        self.post_history_vecs = copy.deepcopy(self.history_vecs)
        self.post_turn_vecs = copy.deepcopy(self.turn_vecs)
        self.post_role_vecs = copy.deepcopy(self.role_vecs)

        _s = [self.dict[self.p1_token]] + self.parse(label)

        self.post_history_vecs.append(_s)
        self.post_turn_vecs += [0] * len(_s)
        self.post_role_vecs += [1] * len(_s)

        self.reverse_post_history_vecs = copy.deepcopy(self.post_history_vecs)

        """
        count = 0
        while self.reverse_post_history_vecs == self.post_history_vecs:
            if count == 2:
                self.reverse_post_history_vecs = self.post_history_vecs[::-1]
                break
            else:
                random.shuffle(self.reverse_post_history_vecs)
                count += 1
        """

        self.reverse_post_turn_vecs = []
        k = len(self.reverse_post_history_vecs) - 1
        for turn in self.reverse_post_history_vecs:
            self.reverse_post_turn_vecs += [k] * len(turn)
            k -= 1

        self.reverse_post_role_vecs = []
        for reverse_turn_id in self.reverse_post_turn_vecs:
            if reverse_turn_id % 2 == 1:
                self.reverse_post_role_vecs.append(0)
            else:
                self.reverse_post_role_vecs.append(1)
    ...
    def update_history(self, obs: Message, temp_history: Optional[str] = None):
        ...
        if 'labels' in obs:
            self._update_post_vecs(obs['labels'][0])
        elif 'eval_labels' in obs:
            self._update_post_vecs(obs['eval_labels'][0])

        self.add_start_end_history_vecs = copy.deepcopy(self.history_vecs)
    ...
    def get_post_history_vec(self):
        """
        Return a vectorized version of the reverse history.
        """
        if len(self.post_history_vecs) == 0:
            return None

        # vec type is a list
        history = []
        for vec in self.post_history_vecs[:-1]:
            history += [vec]
            # history += [self.delimiter_tok]
            history += [[]]
        history += [self.post_history_vecs[-1]]
        history = sum(history, [])

        return history

    def get_reverse_post_history_vec(self):
        """
        Return a vectorized version of the reverse history.
        """
        if len(self.reverse_post_history_vecs) == 0:
            return None

        # vec type is a list
        history = []
        for vec in self.reverse_post_history_vecs[:-1]:
            history += [vec]
            # history += [self.delimiter_tok]
            history += [[]]
        history += [self.reverse_post_history_vecs[-1]]
        history = sum(history, [])

        return history

    def get_add_start_end_history_vec(self):
        """
        Return a vectorized version of the reverse history.
        """
        if len(self.add_start_end_history_vecs) == 0:
            return None

        # vec type is a list
        history = []
        for vec in self.add_start_end_history_vecs[:-1]:
            history += [vec]
            # history += [self.delimiter_tok]
            history += [[]]
        history += [self.add_start_end_history_vecs[-1]]
        history = sum(history, [])

        return history

    def get_post_turn_role_vec(self):
        return self.post_turn_vecs, self.post_role_vecs

    def get_reverse_post_turn_role_vec(self):
        return self.reverse_post_turn_vecs, self.reverse_post_role_vecs

    def get_turn_role_vec(self):
        if self.reversed:
            max_turn = max(self.turn_vecs)
            _turn_vecs = [max_turn + 1 - i for i in reversed(self.turn_vecs)]
            _role_vecs = list(reversed(self.role_vecs))
            if _role_vecs[-1] != 0:
                _role_vecs = [1 - i for i in _role_vecs]
            return _turn_vecs, _role_vecs
        return self.turn_vecs, self.role_vecs
...
class TorchAgent(ABC, Agent):
    ...
    P1_TOKEN = '__newln__'  # '__p1__'
    P2_TOKEN = '__newln__'  # '__p2__'
    ...
    def _set_text_vec(self, obs, history, truncate):
        ...
        if 'text_vec' not in obs:
            ...
            if history_string:
                ...
                obs['turn_vec'], obs['role_vec'] = history.get_turn_role_vec()

                if 'labels' in obs or 'eval_labels' in obs:
                    obs['post_text_vec'] = history.get_post_history_vec()
                    obs['post_turn_vec'], obs['post_role_vec'] = history.get_post_turn_role_vec()
                    obs['reverse_post_text_vec'] = history.get_reverse_post_history_vec()
                    obs['reverse_post_turn_vec'], obs['reverse_post_role_vec'] = history.get_reverse_post_turn_role_vec()

                obs['add_start_end_text_vec'] = history.get_add_start_end_history_vec()
        ...
        if obs.get('text_vec') is not None:
            ...
            turn_vec = self._check_truncate(
                obs['turn_vec'], truncate, truncate_left
            )
            role_vec = self._check_truncate(
                obs['role_vec'], truncate, truncate_left
            )

            if 'labels' in obs or 'eval_labels' in obs:
                post_truncated_vec = self._check_truncate(
                    obs['post_text_vec'], truncate, truncate_left
                )
                post_turn_vec = self._check_truncate(
                    obs['post_turn_vec'], truncate, truncate_left
                )
                post_role_vec = self._check_truncate(
                    obs['post_role_vec'], truncate, truncate_left
                )
                reverse_post_truncated_vec = self._check_truncate(
                    obs['reverse_post_text_vec'], truncate, truncate_left
                )
                reverse_post_turn_vec = self._check_truncate(
                    obs['reverse_post_turn_vec'], truncate, truncate_left
                )
                reverse_post_role_vec = self._check_truncate(
                    obs['reverse_post_role_vec'], truncate, truncate_left
                )
            add_start_end_truncated_vec = self._check_truncate(
                obs['add_start_end_text_vec'], truncate, truncate_left
            )
            ...
            obs.force_set('turn_vec', torch.LongTensor(turn_vec))
            obs.force_set('role_vec', torch.LongTensor(role_vec))

            if 'labels' in obs or 'eval_labels' in obs:
                obs.force_set('post_text_vec', torch.LongTensor(post_truncated_vec))
                obs.force_set('post_turn_vec', torch.LongTensor(post_turn_vec))
                obs.force_set('post_role_vec', torch.LongTensor(post_role_vec))
                obs.force_set('reverse_post_text_vec', torch.LongTensor(reverse_post_truncated_vec))
                obs.force_set('reverse_post_turn_vec', torch.LongTensor(reverse_post_turn_vec))
                obs.force_set('reverse_post_role_vec', torch.LongTensor(reverse_post_role_vec))

            obs.force_set('add_start_end_text_vec', torch.LongTensor(add_start_end_truncated_vec))

            obs.force_set(
                'add_start_end_text_vec', self._add_start_end_tokens(obs['add_start_end_text_vec'], True, True)
            )
    ...
    def batchify(self, obs_batch, sort=False):
        ...
        if any(ex.get('text_vec') is not None for ex in exs):
            ...
            _xs_string = [ex.get('full_text', self.EMPTY) for ex in exs]
        
            _turn = [ex.get('turn_vec', self.EMPTY) for ex in exs]
            turn, turn_lens = self._pad_tensor(_turn)
            _role = [ex.get('role_vec', self.EMPTY) for ex in exs]
            role, role_lens = self._pad_tensor(_role)

            _add_start_end_xs = [ex.get('add_start_end_text_vec', self.EMPTY) for ex in exs]
            add_start_end_xs, add_start_end_x_lens = self._pad_tensor(_add_start_end_xs)
        ...
        if some_labels_avail:
            ...
            _post_xs = [ex.get('post_text_vec', self.EMPTY) for ex in exs]
            post_xs, post_x_lens = self._pad_tensor(_post_xs)
            _post_turn = [ex.get('post_turn_vec', self.EMPTY) for ex in exs]
            post_turn, post_turn_lens = self._pad_tensor(_post_turn)
            _post_role = [ex.get('post_role_vec', self.EMPTY) for ex in exs]
            post_role, post_role_lens = self._pad_tensor(_post_role)

            _reverse_post_xs = [ex.get('reverse_post_text_vec', self.EMPTY) for ex in exs]
            reverse_post_xs, reverse_post_x_lens = self._pad_tensor(_reverse_post_xs)
            _reverse_post_turn = [ex.get('reverse_post_turn_vec', self.EMPTY) for ex in exs]
            reverse_post_turn, reverse_post_turn_lens = self._pad_tensor(_reverse_post_turn)
            _reverse_post_role = [ex.get('reverse_post_role_vec', self.EMPTY) for ex in exs]
            reverse_post_role, reverse_post_role_lens = self._pad_tensor(_reverse_post_role)

            _ys_string = [ex.get(field, self.EMPTY)[0] for ex in exs]
        ...
        return_value['turn_vec'] = turn
        return_value['role_vec'] = role
        return_value['text_string'] = _xs_string
        if some_labels_avail:
            return_value['label_string'] = _ys_string
            return_value['post_text_vec'] = post_xs
            return_value['post_turn_vec'] = post_turn
            return_value['post_role_vec'] = post_role        
            return_value['reverse_post_text_vec'] = reverse_post_xs
            return_value['reverse_post_turn_vec'] = reverse_post_turn
            return_value['reverse_post_role_vec'] = reverse_post_role
        return_value['add_start_end_text_vec'] = add_start_end_xs
